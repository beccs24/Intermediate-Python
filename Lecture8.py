...
- Since we’re inserting elements at the front of the Python list, we need to move the
  existing elements over by one in order to make room for them
    - So the first insertion is cheap and takes one step
    - The 2nd insertion needs to move the first element over by one
      in order to make room for the 2nd element
...
- We can say the number of steps this for loop has is:
    ...
    - Therefore, f1 is O(n2)
'''

'''
Python Lists store items in a CONTIGUOUS MEMORY (one right next to each other)
'''

'''
Binary Search Algorithm
- A useful and performant algorithm to search for an item in a collection
  IF THE ITEMS ARE IN SORTED ORDER
  - Since the collection is in sorted order,
    we can check the middle to see if the item we’re looking for is there
    - If the middle element is the one we’re looking for, then great!
    - If the item we’re looking for is < the middle element,
      then we don’t have to search the right side of the collection
    - If the item we’re looking for is > the middle element,
      then we don’t have to search the left side of the collection
