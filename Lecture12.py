...
- Idea: Similar to Bubble Sort, we make passes through the list and find the
        largest element. We then swap the largest element in the correct place
        (each iteration will place the largest element at the end of the list
         assuming we’re sorting in ascending order)
    - It’s not necessary to swap adjacent elements in like Bubble Sort
    - Think of it as “selecting” the largest element and then
...
- Similar to Bubble Sort, we have to make n-1 comparisons during the
...
- For example, the first element is sorted by default.
    ...
    - Then we check the third element and determine if it goes
      before, in the middle, or after the first two (sorted) elements
    - etc.
- Note that in order to make “room” for the inserted element, we have to shift
  the elements greater than the inserted element up by one

- Analogy: Similar to how one might sort a deck of cards * Work left-to-right,
           take a card and “insert” it into the correct position on the
           left portion of the sorted deck
'''
...
- When we check where to insert an element, we do up to 1 swap on
  the first iteration in order to make room for the element to be inserted
  in the sorted left portion of the list
...
- Let’s also look at the BEST case scenario (the list is already sorted)
    - Here, we still go through n elements, but no swaps occur because
      each position is in the correct place
